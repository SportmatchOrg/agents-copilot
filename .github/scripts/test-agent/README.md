...
**Segunda cuenta (opcional).** El tope `free-models-per-day` de OpenRouter se cobra por **cuenta**, no por modelo: cuando pega, los cuatro modelos de la cadena devuelven 429 en ~300ms y el fallback de modelos no sirve para nada (SPO-197 murió así en la iteración 5 de 15). Con el secret `LLM_API_KEY_FALLBACK` seteado a una key de **otra** cuenta, la cadena reinicia desde el primer modelo con esa key. Solo rota si hubo un 402/429: si la cadena murió por JSON roto, otra cuenta devuelve el mismo JSON roto. Una key más del mismo dueño no compra nada.
